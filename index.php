...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['videoUrl'])) {
    ...
    if ($httpCode === 200 && $ebookFinal) {
        // Limpa marcações de markdown do HTML que o Gemini costuma colocar
        $ebook_html = str_replace(['```html', '
```'], '', $ebookFinal);
    } else {
        // Se o Google rejeitar a requisição, exibe o diagnóstico real
        $erro = "Erro na API do Gemini (Código HTTP: $httpCode). Verifique se a chave está ativa ou se atingiu o limite da cota gratuita.";
    }
}
?>
...
<head>
    ...
    <title>EbookForge - Painel Direto</title>
...
<body class="p-4 md:p-10">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-gray-600">Gere, edite e visualize seu ebook diretamente do seu servidor.</p>
        </div>
        ...
        <!-- Área de Exibição do Ebook Gerado -->
        <?php if ($ebook_html): ?>
            <div class="bg-white p-8 rounded-xl shadow-lg mb-8 prose max-w-none border border-gray-100">
                <?php echo $ebook_html; ?>
            </div>
            <button onclick="window.print()" class="w-full border-2 border-[#5D2A18] sanguine-text font-bold py-3 rounded-lg hover:bg-[#5D2A18] hover:text-white transition shadow-sm">
                EXPORTAR PARA PDF
            </button>
        <?php endif; ?>
